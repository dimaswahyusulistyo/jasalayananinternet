    ...
    if ($foto_size > 2097152) {
       ...
       if ($foto != "") {
          ...
          if(in_array($ekstensi, $ekstensi_izin) === true)  {
            ...
            $get_foto = "SELECT foto FROM barang WHERE id='$id'";
            $data_foto = mysqli_query($koneksi, $get_foto); 
            // Mengubah data yang diambil menjadi Array
            $foto_lama = mysqli_fetch_array($data_foto);

            // Menghapus Foto lama didalam folder FOTO
            unlink("gambar/".$foto_lama['foto']);    

			// Memindahkan File kedalam Folder "FOTO"
            move_uploaded_file($file_tmp, 'gambar/'.$foto_nama_new);

            // Query untuk memasukan data kedalam table SISWA
            $query = mysqli_query($koneksi, "UPDATE barang set nama='$nama', harga='$harga', foto='$foto_nama_new', deskripsi='$deskripsi' WHERE id='$id'");

            // Mengecek apakah data gagal diinput atau tidak
            if($query){
            	// header("location:index.php?pesan=berhasil");
                echo "<script>alert('data jenis member berhasil diupdate.');window.location='index.php';</script>";
            } else {
                // header("location:index.php?pesan=gagal");
                echo "<script>alert('data jenis member gagal diupdate.');window.location='index.php';</script>";
            }
    	...
    	// Apabila tidak ada file yang diupload maka akan menjalankan code dibawah ini
          $query = mysqli_query($koneksi, "UPDATE barang set nama='$nama', harga='$harga', deskripsi='$deskripsi' where id ='$id'");
